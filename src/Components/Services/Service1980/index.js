...
const Service1980 = ({width,img_url,services}) => {
    return (
        <div style={{display: 'flex',alignItems: 'center',flexDirection: 'column',margin:10}}>
            ...
            <Grid container style={{width:width=='xs'?"100%":"70%",marginTop:50}}>
                ...
                <Grid item xs={12} sm={6} md={6} style={{display: 'flex',flexDirection: 'column',justifyContent: 'flex-start'}} >
                    <img src={img_url+"/"+item.image} alt ="service" style={{width:50}}/>
                    <div className="head2" style={{width:"80%",marginTop:20}}>{item.title}</div>
                    <div className="head3" style={{width:"80%",fontSize:17,marinTop:40}}>{item.description}</div>
                </Grid>)}
            </Grid>
        </div>
    )
